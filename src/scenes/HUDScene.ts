...
export class HUDScene extends Phaser.Scene {
  ...
  create(data: { world: World }): void {
    ...
    this.add.rectangle(80, 39, 320, 7, 0x18080d, 0.9).setOrigin(0.5).setScrollFactor(0);
    this.bossBar = this.add.rectangle(80, 39, 1, 7, 0xb31217, 1).setOrigin(0.5).setScrollFactor(0);
  ...
  update(): void {
    ...
    if (b.active) {
      this.bossLabel.setText(`${b.defId.toUpperCase()}  ${Math.ceil(b.hp)}/${b.maxHp}`);
      this.bossBar.width = 320 * Phaser.Math.Clamp(b.hp / b.maxHp, 0, 1);
      this.bossLabel.setVisible(true); this.bossBar.setVisible(true);
    } else {
      this.bossLabel.setVisible(false); this.bossBar.setVisible(false);
    }
